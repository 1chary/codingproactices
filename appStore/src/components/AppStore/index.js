...
class AppStore extends Component {
  state = {activeId: tabsList[0].tabId, textEntered: ''}

  divideBasedOnCategory = () => {
    this.setState({activeId: tabsList.tabId})
  }
  ...
  render() {
    const {activeId, textEntered} = this.state
    const filteredList = this.divide()
    const searchBasedOnInput = filteredList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(textEntered.toLowerCase()),
    )
    ...
    return (
      <div className="container">
        ...
        <ul className="tabItemContainer">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              eachTab={eachTab}
              onChange={this.divideBasedOnCategory}
              update={this.update}
        ...
        <ul className="appItemContainer">
          {searchBasedOnInput.map(eachItem => (
