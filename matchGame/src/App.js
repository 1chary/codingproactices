...
class App extends Component {
  ...
  imageClick = thumbnailUrl => {
    ...
    const check = imagesList.filter(
      eachImage => eachImage.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = check[0]
    if (imageUrl === imgUrl) {
      const newImageUrl =
  ...
  //                    restartGame                                 //
  restartGame = () => {
    this.setState({score: 0, timeInSeconds: 60})
    this.setState(prevState => ({
      isGameRunning: !prevState.isGameRunning,
    }))
    this.intervalId = setInterval(this.decrease, 1000)
    ...
    return (
      <>
        <ul className="navBarContainer">
          ...
          />
          <li className="scoreAndTimerContainer">
            ...
            <img
              ...
              alt="timer"
              className="timerImage" // ................photo..............//
            />
            <p className="scoreValue">{timeInSeconds} sec</p>
          </li>
        </ul>
        <div className="appContainer">
          ...
          {isGameRunning && (
            <div className="insideContainer">
              <img src={imgUrl} className="photo" alt="match" />
              <ul className="tabItemContainer">
                {tabsList.map(eachTab => (
                  <li className="individualTabs" key={eachTab.tabId}>
                    <button
                      className="tabButton"
                      type="button"
                      onClick={() => this.categorizeBasedOnTabId(eachTab.tabId)}
                    >
                      <p
                        className={
                          eachTab.tabId === activeTabId
                            ? `underLinedText`
                            : `tabText`
                        }
                      >
              ...
              <ul className="imageContainer">
                {list.map(eachImage => (
                  <li className="individualImage" key={eachImage.id}>
                    <button
                      type="button"
