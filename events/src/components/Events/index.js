...
class Events extends Component {
  ...
  checkStatus = () => {
    ...
    const checkId = eventsList.find(event => event.id === accessId)
    if (checkId) {
      return checkId.registrationStatus
    }
    return ''
  }

  currentId = id => {
    this.setState({accessId: id})
  }
      ...
      <div className="appContainer">
        ...
        <div className="detailsContainer">
          <ActiveEventRegistrationDetails
            activeRegistrationStatus={this.checkStatus()}
          />
        </div>
      </div>
    )
